...
class Camera {
    constructor(focal_length_mm, film_aperture_width_inches, film_aperture_height_inches, near_clip_dist, resolution_width, resolution_height) {
        ...
        this.far_clip_plane_distance = 100;
    ...
    get_projection_matrix() {
        ...
        let f = this.far_clip_plane_distance;
...
function frame() {
    ...
    // ================================================================================================================
    // Render mesh
    let world_to_cam_mat = camera.get_world_to_camera_matrix();
    ...
    if (mesh != null) {
        ...
        for (let i = 0; i < mesh.triangles.length; i += 3) {
            ...
            let cam0 = a.mult_mat(world_to_cam_mat);
            let cam1 = b.mult_mat(world_to_cam_mat);
            let cam2 = c.mult_mat(world_to_cam_mat);

            // Clip space to NDC space
            let ndc0 = projection_matrix.mult_homogeneous_point(cam0);
            let ndc1 = projection_matrix.mult_homogeneous_point(cam1);
            let ndc2 = projection_matrix.mult_homogeneous_point(cam2);

            // NDC space to raster space
            let r0 = new Vec3(
                (ndc0.x + 1) / 2 * canvas_width,
                (-ndc0.y + 1) / 2 * canvas_height,
                ndc0.z
            );
            let r1 = new Vec3(
                (ndc1.x + 1) / 2 * canvas_width,
                (-ndc1.y + 1) / 2 * canvas_height,
                ndc1.z
            );
            let r2 = new Vec3(
                (ndc2.x + 1) / 2 * canvas_width,
                (-ndc2.y + 1) / 2 * canvas_height,
                ndc2.z
            );
            let triangle_area_doubled = edge(r0, r1, r2);

            let triangle_in_view = (cam0.z < -camera.near_clip_plane_distance && cam0.z > -camera.far_clip_plane_distance) &&
                                   (cam1.z < -camera.near_clip_plane_distance && cam1.z > -camera.far_clip_plane_distance) &&
                                   (cam2.z < -camera.near_clip_plane_distance && cam2.z > -camera.far_clip_plane_distance)
            if (triangle_in_view) {
                ...
                for (let x = r_min_x; x < r_max_x; x++) {
                    for (let y = r_min_y; y < r_max_y; y++) {
                        ...
                        if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
                            ...
                            let cam_point_z = 1/(w0*(1/cam0.z) + w1*(1/cam1.z) + w2*(1/cam2.z));
                            if (cam_point_z <= camera.near_clip_plane_distance) {
                                // This point is in front of the near clipping plane
